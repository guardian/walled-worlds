  ...
  function _fetchKML(map) {
    var path = '/map/kml?mid=' + map.mapurl.split('mid=')[1];
    xml = '';
  ...
  function startDrawing() {
    if (mapResults.length > 0) {
      mapData = mapResults.shift();
      _fetchKML(mapData);
    } else {

      var jsoutput = 'define([],' + JSON.stringify(amdSVGS, null, '  ') + ');';

      fs.writeFile('src/js/app/models/svgs.js', jsoutput, function(err) {
        if(err) {
          console.log(err);
        } else {
...
module.exports.run = fetchMapData;
